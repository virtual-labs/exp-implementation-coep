...
//$("#div123").html("");
	...
	dpt_txt = paper.text(x + 210, y + 500, Math.round(dpt)).attr({ 'font-size': 20 });
...
//  				pc_txt.attr('text',pc);
			...
			function case5(){
				...
				setTimeout(function() { sv_V6.attr('fill','green');
				sv_V1.attr('fill','red');
				sv_V2.attr('fill','red')
				reset = 1; 
				}, time * 2000);
...
//			PT_increment(x,y);
	...
	function DPT_increment(x,y){
		intdpt = 250;
	dpt_interval = setInterval(function() {

			if (dpt < dpt_max) {

				dpt = dpt + intdpt;

				dpt_txt.attr('text',Math.round(dpt));
			}else
			   {
				clearInterval(dpt_interval);
			}
		}, dpttime);		
}
